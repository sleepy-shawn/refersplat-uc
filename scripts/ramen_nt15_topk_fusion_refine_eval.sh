#!/usr/bin/env bash
# Evaluate four refined top-k fusion ramen ablations at 15% negative ratio.
set -euo pipefail
...
MASTER_LOG="$ROOT/ramen_nt15_topk_fusion_refine_eval.log"
...
: > "$MASTER_LOG"
eval_one ramen_nt15_qnt_topk_fusion_qln_nofp --fusion_query_layer_norm
eval_one ramen_nt15_qnt_topk_fusion_stopg_nofp --fusion_detach_pooled_g
eval_one ramen_nt15_qnt_topk_fusion_lam05_nofp
eval_one ramen_nt15_qnt_topk_fusion_lam025_nofp
echo "[$(date +%H:%M:%S)] DONE ramen top-k fusion refine eval" | tee -a "$MASTER_LOG"
